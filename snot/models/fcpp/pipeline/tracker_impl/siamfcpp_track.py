...
from snot.models.fcpp.pipeline.pipeline_base import TRACK_PIPELINES, PipelineBase
from snot.models.fcpp.pipeline.utils import (cxywh2xywh, get_crop,
...
class SiamFCppTracker(PipelineBase):
    ...
    def feature(self, im: np.array, target_pos, target_sz, avg_chans=None):
        ...
        with torch.no_grad():
            data = imarray_to_tensor(im_z_crop).cuda()
    ...
    def track(self,
        ...
        with torch.no_grad():
            score, box, cls, ctr, extra = self._model(
                imarray_to_tensor(im_x_crop).cuda(),
                *features,
                phase=phase_track)
